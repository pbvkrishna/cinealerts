...
<body>
<form action="usersnewsletter.php" method="post">
...
$cities = mysql_query("select * from cities where publish = '1'");
while($city = mysql_fetch_array($cities))
	{
		...
		foreach($movieslist as $movie)
		{
			...
			if($mid)
			{
				$movie_data .=show_email_movie($mid,$city['cid'])."<br/>";
			}
		...
		if($movie_data)
		{
			...
			$i=0;
			$users = mysql_query("select * from users where city = '".$city['city']."'");
			while($user = mysql_fetch_array($users))
			{
				$user_email=$user['email'];
				$user_name=$user['name'];
				$subject = "Movies Playing in ".$city['city']." Theaters.";
				if(trim($user_email))
				{
					$query = "insert into emails values('','CineAlerts.com','alerts@cinealerts.com','".$user_name."','".$user_email."','".$subject."','".addslashes($movie_data)."')";
					mysql_query($query);
					$i++;
				}
			}
	
			echo $city['city']." - ".$i." - Emails qued<br/>";
		}
	}
}
