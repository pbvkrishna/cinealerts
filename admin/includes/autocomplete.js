...
var lblock;
var ti;
	function autoshow(idname,data){
	
	
	AutoComplete_Create(idname, data,pname,param);


//	 AutoComplete_Create(idname, data);
}
    ...
    */
    function AutoComplete_Create (id, data,pname1,param1,cblock,param2)
    {
	if(!cblock)	
	{
		lblock = 'contentblock';
	...
	param=param1;
        ...
        if (arguments[5] != null) {
    	    __AutoComplete[id]['maxitems'] = arguments[5];
            __AutoComplete[id]['firstItemShowing'] = 0;
            __AutoComplete[id]['lastItemShowing']  = arguments[5] - 1;
        }
    ...
    */
    function AutoComplete_CreateDropdown(id)
    {
		...
		if(document.getElementById(id).name=="descp"){
		__AutoComplete[id]['dropdown'].style.width      = 780+ 'px';
		}else{
        __AutoComplete[id]['dropdown'].style.width      = width + 'px';
		}
        __AutoComplete[id]['dropdown'].style.zIndex     = '300';
    ...
    */
    ...
    {
		if(document.getElementById("toolreceiptsdiv")){
			
				var divscrollvalue=document.getElementById("toolreceiptsdiv").scrollLeft;
		} else{
        ...
        for (i=0; i<__AutoComplete[id]['data'].length; ++i) {
            ...
            if (__AutoComplete[id]['data'][i].substr(0, value.length).toUpperCase() == value.toUpperCase() || __AutoComplete[id]['data'][i].substr(0, value.length).toLowerCase() == value.toLowerCase()) {
        ...
        for (i=0; i<toDisplay.length; ++i) {
            ...
            newDiv.onclick     = function() {AutoComplete_SetValue(__AutoComplete[id]['element'].getAttribute('id')); AutoComplete_HideDropdown(__AutoComplete[id]['element'].getAttribute('id')); ajaxFunction(pname,'new',param,lblock,'0');}
            //document.getElementById("sivnf").submit();
        ...
        */
		...
		if(document.getElementById(id).name=="descp"){
		__AutoComplete[id]['dropdown'].style.left = 250+"px";
			
		}else{
				
				if(document.getElementById("toolreceiptsdiv")){
					elementsnu=document.getElementById("toolreceiptsdiv").getElementsByTagName("input").length;
					var element=false;
					for (i=0; i<elementsnu;i++){
						
						if(document.getElementById("toolreceiptsdiv").getElementsByTagName("input")[i].name==document.getElementById(id).name){
							element=true;
						}
			...
			if(element){
				        __AutoComplete[id]['dropdown'].style.left = parseInt(AutoComplete_GetLeft(__AutoComplete[id]['element'])-divscrollvalue)+"px";
			}else{
				        __AutoComplete[id]['dropdown'].style.left = parseInt(AutoComplete_GetLeft(__AutoComplete[id]['element']))+"px";
			}
    ...
    */
    function AutoComplete_ScrollCheck(id)
    {
        ...
        if (__AutoComplete[id]['highlighted'] > __AutoComplete[id]['lastItemShowing']) {
            __AutoComplete[id]['firstItemShowing'] = __AutoComplete[id]['highlighted'] - (__AutoComplete[id]['maxitems'] - 5);
    ...
    */
    function AutoComplete_KeyDown(id)
    {
		...
		switch (keyCode) {
            ...
            // Return/Enter
            case 13:
var ti=document.getElementById(id).tabIndex;
...
if(lblock==''){
		...
		ajaxFunction(pname,'new',param,'contentblock',ti);
}
						else
						{
							ajaxFunction(pname,'new',param,lblock,ti);
						}
				...
				else
				{
					...
					if(lblock!='donot'){
						...
						{
							document.forms[0].elements[ti+2].focus()
						}
